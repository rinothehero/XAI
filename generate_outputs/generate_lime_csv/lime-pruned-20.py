...
# 2. Load pruned model
#pruned_model = AutoModelForSequenceClassification.from_pretrained("../pruned_bert_agnews_structured-20-7", local_files_only=True)
pruned_model = torch.load("../shrunk_bert_state.pt")
pruned_model.to('cuda:0').eval()
...
def predict_proba(texts):
    ...
    keep_long = {"input_ids", "token_type_ids"}
    inputs = {
        k: (v.to("cuda:0") if k in keep_long else v.half().to("cuda:0"))
...
output_file = "../quantized-bert/lime_text_only.csv"
...
output_df.to_csv("lime_explanation-pruned-20.csv", index=False)
