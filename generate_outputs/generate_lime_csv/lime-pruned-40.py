...
# 2. Load pruned model
pruned_model = AutoModelForSequenceClassification.from_pretrained("../pruned_bert_agnews_structured-40-1", local_files_only=True)
...
def predict_proba(texts):
    ...
    inputs = {
        k: (v.to("cuda:1") if k in keep_long else v.half().to("cuda:1"))
...
output_df.to_csv("lime_explanation-pruned-40-1.csv", index=False)
